...
async function seed() {
  ...
  // 2. Add fake products
  const { data: prodData, error: err1 } = await supabase.from('products').upsert([
  ...
  const { error: err2 } = await supabase.from('farmer_shop_products').upsert([
  ...
  const { data: fsData, error: err3 } = await supabase.from('fast_sell').upsert([
  ...
  // 5. Add fake Orders
  const { error: err4 } = await supabase.from('orders').upsert([
